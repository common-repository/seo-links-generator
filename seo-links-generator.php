<?php
/*
Plugin Name: SEO Links Generator
Plugin URI: http://www.eryushion.com/wordpress-plugins/seo-links-generator/
Version: 1.0
Author: Vimal Ghorecha
Author URI: http://www.vhghorecha.in
Description: With SEO Links Generator you can easily add links (automatically) for keywords and phrases in posts, pages and comments.
*/

if ( !class_exists('SEOLinksGenerator') ) :
	
class SEOLinksGenerator {
	var $SEOLinksGenerator_DB_option = 'SEOLinksGenerator';
	var $SEOLinksGenerator_options;
	
	function SEOLinksGenerator() {
	  $options = $this->get_options();
	  if ($options) {
			if ($options['post'] || $options['page'])		
				add_filter('the_content',  array(&$this, 'SEOLinksGenerator_the_content_filter'), 10);
			if ($options['comment'])						
				add_filter('comment_text', array(&$this, 'SEOLinksGenerator_comment_text_filter'), 10);
		}
		add_action('create_category', array(&$this, 'SEOLinksGenerator_delete_cache'));
		add_action('edit_category', array(&$this,'SEOLinksGenerator_delete_cache'));
		add_action('edit_post', array(&$this,'SEOLinksGenerator_delete_cache'));
		add_action('save_post', array(&$this,'SEOLinksGenerator_delete_cache'));
		add_action('admin_menu', array(&$this, 'SEOLinksGenerator_admin_menu'));
		load_plugin_textdomain('seo-links-generator', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/');
		add_action('admin_enqueue_scripts', array(&$this, 'SEOLinksGenerator_admin_menu_load_scripts'));
	}

function SEOLinksGenerator_process_text($text, $mode) {
    ...
    if (!empty($options['customkey_url'])) {
        ...
        if ($now - $last_update > 86400) {
            ...
            update_option($this->SEOLinksGenerator_DB_option, $options);
        }
	...
	if ($options['lposts'] || $options['lpages']) {
		if ( !$posts = wp_cache_get( 'seo-links-posts', 'seo-links-generator' ) ) {
			...
			wp_cache_add( 'seo-links-posts', $posts, 'seo-links-generator', 86400 );
		}
	...
	// categories
	if ($options['lcats']) {
		if ( !$categories = wp_cache_get( 'seo-links-categories', 'seo-links-generator' ) ) {
			...
			wp_cache_add( 'seo-links-categories', $categories, 'seo-links-generator',86400 );
		}
	...
	// tags
	if ($options['ltags']) {
		if ( !$tags = wp_cache_get( 'seo-links-tags', 'seo-links-generator' ) ) {
			...
			wp_cache_add( 'seo-links-tags', $tags, 'seo-links-generator',86400 );
		}
...
function SEOLinksGenerator_the_content_filter($text) {
	return SEOAutoTextFilter($this->get_options(),$this->SEOLinksGenerator_process_text($text, 0));
}

function SEOLinksGenerator_comment_text_filter($text) {
	return SEOAutoTextFilter($this->get_options(),$this->SEOLinksGenerator_process_text($text, 1));
}
...
function get_options() {
	...
	$saved = get_option($this->SEOLinksGenerator_DB_option);
...
function install() {
	$SEOLinksGenerator_options = $this->get_options();
}

function uninstall(){
	$this->SEOLinksGenerator_delete_cache(0);
	delete_option($this->SEOLinksGenerator_DB_option);
}
...
function handle_options() {
	...
	if (isset($_POST['submitted']) ) {
		...
		check_admin_referer('seo-links-generator');
		
		if($_POST['resetdata'] == 'on'){
			$options = array(
				'post' => 'on',
				'postself' => '',
				'page' => 'on',
		...
		_e('Plugin settings saved.','seo-links-generator');
		echo '</p></div>';
	}
	...
	$nonce			= wp_create_nonce('seo-links-generator');
	
	/**
	 * Require admin settings page
	 */
	require_once dirname(__FILE__) . '/seo-links-generator-admin.php';
}
	
function SEOLinksGenerator_admin_menu() {
	global $seoauto_adminmenu;
	$seoauto_adminmenu = add_options_page('SEO Links Generator Options', 'SEO Links Generator', 8, basename(__FILE__), array(&$this, 'handle_options'));
}

function SEOLinksGenerator_admin_menu_load_scripts($hook) {
	global $seoauto_adminmenu;
	if( $hook != $seoauto_adminmenu ) 
		return;
	wp_enqueue_script('tagsjs', plugins_url( '/js/load.js', __FILE__ ) );
	wp_enqueue_style('tagscss', plugins_url( '/css/seo-links-generator-style.css', __FILE__ ) );
}

function SEOLinksGenerator_delete_cache($id) {
	wp_cache_delete( 'seo-links-categories', 'seo-links-generator' );
	wp_cache_delete( 'seo-links-tags', 'seo-links-generator' );
	wp_cache_delete( 'seo-links-posts', 'seo-links-generator' );
}
...
if (class_exists('SEOLinksGenerator') ) :
	$SEOLinksGenerator = new SEOLinksGenerator();
	if (isset($SEOLinksGenerator)) {
		register_activation_hook( __FILE__, array(&$SEOLinksGenerator, 'install') );
		register_uninstall_hook( __FILE__, array(&$SEOLinksGenerator, 'uninstall') );
	}
endif;

	/**
	 * Require SEO Links Generator functions
	 */
	require_once dirname(__FILE__) . '/seo-links-generator-functions.php';
?>